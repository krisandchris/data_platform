...
The import reads from required CLI flags:

- `--platform-state-root` for file-backed platform state;
- `--label-config-store-root` for label configs and dataset registries;
- `--dataset-root` for readonly source dataset references.
