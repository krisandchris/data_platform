...
| `--platform-state-root` / `PLATFORM_STATE_ROOT` | `/data/platform_state` in the backend container | Source file-backed runtime state root read by the import command. |
| `--label-config-store-root` / `LABEL_CONFIG_STORE_ROOT` | `/data/label_config_state` in the backend container | Source label config and dataset registry root read by the import command. |
| `--dataset-root` / `DATASET_ROOT` | `/data/datasets/urban_violation` in the backend container | Readonly source dataset root used to preserve filesystem references. |

Expected Phase 6 command surface, pending final Backend Agent implementation handoff:
