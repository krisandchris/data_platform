...
Backend-confirmation-dependent expected command name:

```bash
uv run python -m urban_violation_backend.migrate_state import-file-state
```

The backend Phase 6 branch must confirm this exact command or hand off the replacement before production use. At this docs checkpoint, the Backend Agent handoff still lists the command above as the expected surface.
...
Backend-confirmation-dependent: this section documents the expected Phase 6 CLI surface from the Backend Agent assignment handoff. Reconcile exact flags and report schema with the backend and QA handoffs before Lead Agent integration.
...
| `--platform-state-root` or `PLATFORM_STATE_ROOT` | Dry-run and apply | Source file-backed state root containing users, sessions, audit, QC, drafts, submissions, sample pool, exports, and evaluations. |
| `--label-config-store-root` or `LABEL_CONFIG_STORE_ROOT` | Dry-run and apply | Source label config and dataset registry state root. |
| `--dataset-root` or `DATASET_ROOT` | Dry-run, apply, and post-import runtime verification | Readonly source dataset root, for example `/srv/urban-platform/DATASET/urban_violation`. |
...
Phase 6 expected workflow. The file-state import tool is not part of Phase 3 PostgreSQL foundation, Phase 4 QC/review state migration, or Phase 5 Redis runtime coordination. Do not run a production import until the backend import-tool phase has landed, QA import coverage has passed, and the Lead Agent has reconciled the final command name.
...
Conflict reports should identify the state class, stable id or natural key, source digest or content summary, target digest or content summary, and recommended operator action when the backend implementation can provide those fields.

## 6. Import File State

Phase 6 expected workflow. This is blocked until the file-state import tool exists and has passed dry-run, idempotency, and conflict-report tests.
