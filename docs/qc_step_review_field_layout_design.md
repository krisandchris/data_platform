...
│ 裁决条：通过 / 需修改 / 驳回 / 人工精标 + vote note + 保存/提交           │
└─────────────────────────────────────────────────────────────────────────┘
```
...
- 左侧图像证据区占 60%-65% 宽度。
- 右侧审阅栏占 35%-40% 宽度。
- 右侧上区 `Relation 复核区` 占右栏 58%-65% 高度。
- 右侧下区 `Candidate 与质检裁决` 占右栏 35%-42% 高度。
- 底部 vote note 与裁决按钮保持贴近浏览器底部；若实现为全局底栏，应视觉上归属 `Candidate 与质检裁决`。
